<template>
  <div class="app-container">
    ...
    <!-- 表格 -->
    <el-table v-loading="loading" :data="mortgageList">
...
import { mortgageList } from '@/api/process/mortgageAudit'
...
export default {
  ...
  watch: {
    $route(to, from) {
      //监听路由是否变化
      if (to.path == '/mortgage/mortgageAudit') {
        this.getList()
      }
    },
  },
  methods: {
    ...
    getList() {
      ...
      mortgageList(this.queryParams).then((response) => {
        this.mortgageList = response.rows
    ...
    async handle(item) {
      try {
        this.$router.push({
          path: 'mortgage/mortgageAuditDetails',
