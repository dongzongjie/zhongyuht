<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="businessList">
      ...
      <el-table-column label="当前操作人" align="center" prop="grantBy" />
      <el-table-column
        label="操作"
        align="center"
      ...
      >
        <template slot-scope="scope">
          ...
          <div v-if="scope.row.submitState">
            ...
            <div v-else-if="scope.row.grantId == $store.state.user.userId">
              <el-button size="mini" type="text" @click="handle(scope.row)"
                >立即处理</el-button
              >
  ...
  methods: {
    ...
    async handle(item) {
      try {
        ...
        this.getList()
        this.$router.push({
          path: '/process/CreditExtensionDetails',
    ...
    myCustomer() {
      this.$router.push({
        ...
        name: 'MyCustomer',
        query: {
          flow: 'creditExtension',
          flowName: 'CreditExtension',
        },
      })
    },
  },
